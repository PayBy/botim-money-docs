...
If the payment result is not returned for a long time, you can use this interface to cancel the order. However, Botim Money only supports cancellations when the order is in the `CREATED` status, and the order status will be updated to `FAILURE` after Botim Money processed the cancellation request successfully.

If the order status has been updated to `PAID_SUCCESS` or `SETTLED` when the cancellation was initiated, Botim Money will reject this  request. It is recommended to use `Revoke` or `Refund` interface.  
...
| 62035    | ORDER_NO_NOT_EXIST          | Botim Money order number does not exist                | Check and correct the Botim Money order number     |
