...
If the payment result is not returned for a long time, you can use this interface to cancel the order. However, Botim Money Business only supports cancellations when the order is in the `CREATED` status, and the order status will be updated to `FAILURE` after Botim Money Business processed the cancellation request successfully.

If the order status has been updated to `PAID_SUCCESS` or `SETTLED` when the cancellation was initiated, Botim Money Business will reject this  request. It is recommended to use `Revoke` or `Refund` interface.  
...
- **bizContent** <font color='#7d8793'>Object</font>  
  ...
  - **orderNo** <font color='#7d8793'>String</font> <font color='#f19938'>Required</font>  
    - The Botim Money Business's unique identification number of the order.  
...
| 404      | SERVICE_NOT_AVAILABLE       | API service unavailable                          | Contact Botim Money Business                          |
...
| 62035    | ORDER_NO_NOT_EXIST          | Botim Money Business order number does not exist                | Check and correct the Botim Money Business order number     |
