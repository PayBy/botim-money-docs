...
import PREAUTH from "./OrderCreation/_preauth.md";
import PREAUTHVOID from "./OrderCreation/_preauthvoid.md";
import CAPTURE from "./OrderCreation/_capture.md";
...
[PRODUCTS_ENUM.PREAUTH]:PREAUTH,
[PRODUCTS_ENUM.PREAUTHVOID]:PREAUTHVOID,
[PRODUCTS_ENUM.CAPTURE]:CAPTURE
};
...
export function SwitchProduct() {
  ...
  return (<div>
      ...
      <br />
      <br />
​      <b>paySceneParams</b> <span style={{ color: " #7d8793" }}>Object</span>
​      <details open={true}>
​        <summary>
​          Attributes
​        </summary>
...
The merchant system first calls this API to generate a prepayment order in the Botim Money service backend. Once a valid token (order session identifier) is returned, the payment process can then be initiated.
...
- **subjectAr** <span style={{color:" #7d8793"}}>String</span> 
  - Arabic description of this order.
  - Example value: آيفون
  - Maximum length: `200`

- **riskInfo** <span style={{color:" #7d8793"}}>String</span> 
  - Risk parameters in JSON format
...
- **paySceneCode** <span style={{color:" #7d8793"}}>Enum</span> <span style={{color:"#f19938"}}>Required</span>
    ...
    Payment scene you are going to use. The possible values are `PAYPAGE`, `INAPP`, `EWALLET`, `DYNQR`, `QRPAY`, `JSAPI`, `AUTODEBIT`, `DIRECTPAY`, `PREAUTH`, `PREAUTHVOID`, and `CAPTURE`.

    Select from the drop down to get parameters that need to be passed in different scenarios.

    <SwitchProduct />

- **agreementInfo** <span style={{color:" #7d8793"}}>Object</span> <span style={{color:"#f19938"}}>Required</span>

    > **Note:** The AgreementInfo parameters are applicable for Case 1, Case 2, and Case4 in DIRECTPAY for the time being. They are **NOT applicable** for Case3 (Installment payment). These parameters are only effective when `agreementSign` is set to **True**.
...
- **sharingParamList** <span style={{color:" #7d8793"}}>List</span> 
    ...
    With every payment, you can split the funds between your merchant account and other Botim Money member IDs. For example, a payment of 10 AED, if you share 1 AED to A and 2 AED to B, you will get 7 AED.
  ...
  - **sharingIdentityType**   <font color = ' #7d8793'>String</font>    <font color = '#f19938'>Required</font>
    ...
    The fund can be transferred to the beneficiary through mobile phone number, Botim Money member id or BOTIM member id.
  ...
  - **withholdAndRemitFee**   <font color = ' #7d8793'>Boolean</font>    
    ...
    Whether this sharing account pay the Botim Money transaction fee. If `true`, the final amount this sharing account received is : 
    `[**sharingAmount** - Botim Money transaction fee]`.
...
- **secondaryMerchantId** <span style={{color:" #7d8793"}}>String</span>  
  ...
  > **Note:** *To learn more about this feature please contact Botim Money. *
...
- **reserved** <span style={{color:" #7d8793"}}>String</span>  
  ...
  - Reserved field for passing additional order information or merchant-defined metadata
  - Maximum length: <code>20</code>
...
Http Body
{
    ...
    "bizContent":{
        "riskInfo": {"scene": "casino", "level": "low" },
...
- **interactionParams**
  ...
  In different payment scenarios, Botim Money may return different parameters, allowing the merchant system and Botim Money continue to interact to complete the order.
  ...
  - **deepLink** <span style={{color:" #7d8793"}}>String</span>
    ...
    When the payer chooses to open another e-wallet for payment at the merchant's payment page, Botim Money will return a link to redirect the payer to the e-wallet's checkout.


  - **threeDSecureDom** <span style={{color:" #7d8793"}}>String</span>

    Appears only in `DIRECTPAY` and `PREAUTH` payment scene.

    If the merchant decides or Botim Money's risk control system recognizes that the transaction requires 3DS verification, the 3DS verification link will be returned.
...
| 404   | SERVICE_NOT_AVAILABLE                        | API service not available                                    | Contact Botim Money                               |
...
| 62086 | INVALID_PAYEEMID                             | Invalid payeeMid                                             | Contact Botim Money                               |
