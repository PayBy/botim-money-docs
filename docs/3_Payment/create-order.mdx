...
The merchant system first calls this API to generate a prepayment order in the Botim Money Business service backend. Once a valid token (order session identifier) is returned, the payment process can then be initiated.
...
- **sharingParamList** <span style={{color:" #7d8793"}}>List</span> 
    ...
    With every payment, you can split the funds between your merchant account and other Botim Money Business member IDs. For example, a payment of 10 AED, if you share 1 AED to A and 2 AED to B, you will get 7 AED.
  ...
  - **sharingIdentityType**   <font color = ' #7d8793'>String</font>    <font color = '#f19938'>Required</font>
    ...
    The fund can be transferred to the beneficiary through mobile phone number, Botim Money Business member id or BOTIM member id.
  
    The possible values are: 
  
    `PHONE_NO`. Transfer through mobile phone number.
  
    `MEMBER_ID`. Transfer through Botim Money Business member id.  
  ...
  - **withholdAndRemitFee**   <font color = ' #7d8793'>Boolean</font>    
    ...
    Whether this sharing account pay the Botim Money Business transaction fee. If `true`, the final amount this sharing account received is : 
    `[**sharingAmount** - Botim Money Business transaction fee]`.
...
- **secondaryMerchantId** <span style={{color:" #7d8793"}}>String</span>  
  ...
  > **Note:** *To learn more about this feature please contact Botim Money Business. *
...
When Botim Money Business sends response, Botim Money Business will use its own private key to sign the message, and the merchant uses Botim Money Business 's public key to verify the signature. If the verification is passed, it proves that the response was sent by Botim Money Business and not faked by others.
...
- **interactionParams**
  ...
  In different payment scenarios, Botim Money Business may return different parameters, allowing the merchant system and Botim Money Business continue to interact to complete the order.
  ...
  - **deepLink** <span style={{color:" #7d8793"}}>String</span>
    ...
    When the payer chooses to open another e-wallet for payment at the merchant's payment page, Botim Money Business will return a link to redirect the payer to the e-wallet's checkout.
  ...
  - **threeDSecureDom** <span style={{color:" #7d8793"}}>String</span>
    ...
    If the merchant decides or Botim Money Business's risk control system recognizes that the transaction requires 3DS verification, the 3DS verification link will be returned.
...
| 403   | UNAUTHORIZED                                 | API not authorized                                           | Contact Botim Money Business                               |
| 404   | SERVICE_NOT_AVAILABLE                        | API service not available                                    | Contact Botim Money Business                               |
| 500   | SYSTEM_ERROR                                 | System error                                                 | Contact Botim Money Business. Retry later               |
...
| 62086 | INVALID_PAYEEMID                             | Invalid payeeMid                                             | Contact Botim Money Business                               |
