---
sidebar_position: 4
toc_max_heading_level: 6
---

import RequestHeader from '../_request-header.mdx'
import ResponseHead from './_response-header.mdx'


# Query VAM Xtran

## Overview

The `queryVamXtran` interface enables merchants to query previously applied VAM (Virtual Account Management) accounts by reference number and/or account holder name. At least one of `referenceNumber` and `accountHolderName` must be provided. Botim Money returns a list of matching virtual account information.


## API URL

- UAT: https://uat.test2pay.com/sgs/api/vam/queryVamXtran
- Production: https://api.payby.com/sgs/api/vam/queryVamXtran


## Request

### Http Header

<details>
  <summary>Attributes</summary>
  <RequestHeader/>
</details>


### Http Body


- **requestTime**   <font color = ' #7d8793'>Timestamp(3)</font>   <font color = '#f19938'>Required</font>

	- Example value: 1581493898000

- **bizContent**   <font color = ' #7d8793'>Object</font>   <font color = '#f19938'>Required</font>

	- **referenceNumber**   <font color = ' #7d8793'>String</font>    <font color = '#f19938'>Optional</font>
		- Merchant-defined reference number. Only letters and digits are supported. At least one of `referenceNumber` and `accountHolderName` must be provided.
		- Example value: REFQUERY001
		- Maximum length: `64`

	- **accountHolderName**   <font color = ' #7d8793'>String</font>    <font color = '#f19938'>Optional</font>
		- Account holder name. Encrypted with the merchant's public key (SHA-256). At least one of `referenceNumber` and `accountHolderName` must be provided.
		- Maximum length: `75`


### Request Sample

```json
//Http Header
{
    "Content-Language": "en",
    "Content-Type": "application/json",
    "sign": "",
    "Partner-Id": "200000429066"
}

//Http Body
{
  "requestTime": 1581405399656,
  "bizContent": {
    "referenceNumber": "REFQUERY001",
    "accountHolderName": ""
  }
}
```


## Response

### Http Header


**sign**   <font color = ' #7d8793'>String</font>   <font color = '#f19938'>Required</font>

When Botim Money sends response, Botim Money will use its own private key to sign the message, and the merchant uses Botim Money 's public key to verify the signature. If the verification is passed, it proves that the response was sent by Botim Money and not faked by others.


### Http Body

**head** 

  <details>
    <summary>Attributes</summary>
    <ResponseHead/>
  </details>

**body** 


**<font color = '#f19938'>Notice </font>**

Body is returned only when `applyStatus` = `SUCCESS` and `code` = `0`.

- **vamAccountDetails** <font color='#7d8793'>List&lt;VamAccountDetail&gt;</font> <font color='#f19938'>Required</font>  
  - List of virtual account information.

**VamAccountDetail**

- **iban** <font color='#7d8793'>String</font> <font color='#f19938'>Required</font>  
  - Virtual account IBAN. Returned encrypted with the merchant's public key (SHA-256).

- **accountNo** <font color='#7d8793'>String</font> <font color='#f19938'>Required</font>  
  - Virtual account number.  
  - Example value: `600001603`

- **referenceNumber** <font color='#7d8793'>String</font> <font color='#f19938'>Required</font>  
  - Merchant-defined reference number.  
  - Example value: `REFQUERY001`

- **swiftCode** <font color='#7d8793'>String</font> <font color='#f19938'>Required</font>  
  - Bank SWIFT/BIC code.  
  - Example value: `BBMEAEADXXX`

- **bankAddress** <font color='#7d8793'>String</font>  
  - Bank address.  
  - Example value: `Abu Dhabi, UAE`

- **accountStatus** <font color='#7d8793'>String</font> <font color='#f19938'>Required</font>  
  - Virtual account status.  
  - Possible values: `Init`, `Processing`, `Valid`, `Closed`, `Failed`  
  - Example value: `Valid`

- **accountHolder** <font color='#7d8793'>String</font> <font color='#f19938'>Required</font>  
  - Account holder name. Returned encrypted with the merchant's public key (SHA-256).

- **bankName** <font color='#7d8793'>String</font> <font color='#f19938'>Required</font>  
  - Bank name.  
  - Example value: `Mashreq Bank`

### Response Sample

```json
//Http Header
{
  "sign": ""
}

//Http Body
{
  "head": {
    "applyStatus": "SUCCESS",
    "code": "0",
    "msg": "SUCCESS"
  },
  "body": {
    "vamAccountDetails": [
      {
        "iban": "",
        "accountNo": "600001603",
        "referenceNumber": "REFQUERY001",
        "swiftCode": "BBMEAEADXXX",
        "bankAddress": "Abu Dhabi, UAE",
        "accountStatus": "Valid",
        "accountHolder": "",
        "bankName": "Mashreq Bank"
      }
    ]
  }
}
```

## Response Codes


| **Code** | **Message**            | **Cause**                                                                 | **Workaround**                       |
|---------|------------------------|---------------------------------------------------------------------------|--------------------------------------|
| 0       | SUCCESS                | Success                                                                    |                                      |
| 400     | INVALID_PARAMETER      | Invalid parameters, for example both `referenceNumber` and `accountHolderName` are empty | Adjust request parameters            |
| 402     | RATE_LIMIT_REJECT      | Too many requests                                                          | Reduce request frequency             |
| 403     | UNAUTHORIZED           | API not authorized, signature error, or KYC not passed                     | Contact Botim Money                  |
| 404     | SERVICE_NOT_AVAILABLE  | API service unavailable                                                    | Contact Botim Money                  |
| 500     | SYSTEM_ERROR           | System error                                                               | Contact Botim Money, or try again later |
| 504     | SERVICE_TIMEOUT        | Service timeout                                                            | Try again later                      |
| 62004   | MERCHANT_ORDER_NO_NOT_EXIST | No virtual account record found for the given reference number         | Adjust request parameters            |
| 62101   | WRONG_IBAN_FORMAT      | IBAN is empty or incorrectly formatted                                     | Contact Botim Money                  |
| 62102   | IBAN_NOT_FOUND         | IBAN not found                                                             | Contact Botim Money                  |
| 62118   | INVALID_RESPONSE       | Invalid response returned by the channel                                   | Contact Botim Money, or try again later |
