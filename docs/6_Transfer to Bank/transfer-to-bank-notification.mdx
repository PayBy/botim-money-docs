 ...
 **<font color = '#f19938'>Note</font>**
  ...
  4. Signature Verification: Notification requests are signed using PayBy’s RSA private key. Merchants must verify the signature using the Botim Money Business public key, which can be downloaded from the merchant portal. The signing method is the same as for regular API requests.
