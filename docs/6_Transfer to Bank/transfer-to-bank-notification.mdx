...
Once a transfer to a bank account is processed, Botim Money Business will send the result to the merchant. The merchant must handle the notification and respond accordingly. The actual time the funds arrive depends on the settlement times of the receiving bank.
 ...
 **<font color = '#f19938'>Note</font>**
  ...
  2. Retry Mechanism: If Botim Money Business does not receive a valid or timely response from your system, it will consider the notification failed and retry sending it. Botim Money Business will retry sending the notification up to 7 times at intervals of 2, 10, 10, 60, 120, 360, and 900 minutes, but final delivery success is not guaranteed.
  ...
  4. Signature Verification: Notification requests are signed using Botim Money Business’s RSA private key. Merchants must verify the signature using the Botim Money Business public key, which can be downloaded from the merchant portal. The signing method is the same as for regular API requests.
...
- **notify_timestamp**  <font color = ' #7d8793'>Timestamp</font>  <font color = '#f19938'>Required</font>
  ...
  The timestamp when Botim Money Business sent the request.
...
- **notify_id**  <font color = ' #7d8793'>String</font>  <font color = '#f19938'>Required</font>
  ...
  The unique identification number of this notification within the Botim Money Business system.
...
Please reply `success` after receiving the notification, otherwise Botim Money Business will send the notification repeatedly for the same order.
...
| 403    | UNAUTHORIZED                     | API access not authorized                                                   | Contact Botim Money Business                |
| 404    | SERVICE_NOT_AVAILABLE           | API service is currently unavailable                                        | Contact Botim Money Business                |
| 500    | SYSTEM_ERROR                    | Internal system error                                                       | Contact Botim Money Business and retry later|
...
| 62076  | FUND_OUT_ABILITY_SUPPORTED      | `networkCode` or `fundoutCurrencyCode` not enabled or unsupported           | Contact Botim Money Business                |
