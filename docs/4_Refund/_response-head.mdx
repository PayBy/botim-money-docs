
- **applyStatus** <font color='#7d8793'>Enum</font> <font color='#f19938'>Required</font>  
  - The result of the request. Possible values:  
    - `SUCCESS`: Application successful.  
    - `FAIL`: Application failed. Check the `code` and `msg` for the exact reason.  
    - `ERROR`: Application error. Signature verification failed. Ensure the private key used for the signature and the public key uploaded on the Botim Money Business portal are a valid key pair.

- **code** <font color='#7d8793'>String</font> <font color='#f19938'>Required</font>  
  - Response code.  
  - Example value: `0`  
  - Maximum length: `10`

- **msg** <font color='#7d8793'>String</font>  
  - Description of the response code.  
  - Maximum length: `200`

- **traceCode** <font color='#7d8793'>String</font>  
  - Internal code for locating the error.
